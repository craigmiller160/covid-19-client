...
export const loadCountries = () => async (dispatch) => {
	try {
		...
		dispatch(
			countryDataSlice.actions.setCountries(formatCountryData(res.data))
		);
	} catch (ex) {
...
export const loadCountryHistoricalData = ({ field, value } = {}) => async (
	dispatch,
	getState
) => {
	try {
		...
		if (values.location.value === worldOption.value) {
			res = await getWorldHistoricalData(
				values.startDate,
				values.endDate
			);
		} else {
...
export const loadCountryCompareData = () => async (dispatch) => {
	try {
		dispatch(coreSlice.actions.setLoading(true));
...
export const loadCountryCurrentData = ({ field, value } = {}) => async (
	dispatch,
	getState
) => {
	try {
		...
		const { sortKey, sortOrder } =
			getState().form[COUNTRY_COMPARE_FORM]?.values ?? {};
		...
		const res = await getCountryCurrentData(
			realSortKey.value,
			realSortOrder.value
		);
